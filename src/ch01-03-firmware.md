...
New versions of PLCnext Control firmware are released regularly. If the firmware on your PLC is not the latest, you may want to upgrade it. This can be achieved using the following steps:
...
The PLC will take some time to install the new firmware, during which time the PLC will restart.




### Open a Secure Shell Session to the PLC

The PLC is running a secure shell (ssh) daemon, so we can go ahead and open a shell session on the PLC from the host:
